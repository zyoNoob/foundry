...
def benchmark_generation_speed(base_url, num_requests=5, max_tokens=128):
...
def benchmark_prompt_processing(base_url, prompt_sizes=[100, 500, 1000]):
...
def benchmark_concurrent_throughput(base_url, num_concurrent=4, num_iterations=3):
...
def main():
    ...
    parser.add_argument("--mode", choices=["all", "generation", "prompt", "throughput"], 
    ...
    if args.mode in ["all", "generation"]:
        results["generation"] = benchmark_generation_speed(args.url, num_requests=args.requests)
    
    if args.mode in ["all", "prompt"]:
        results["prompt_processing"] = benchmark_prompt_processing(args.url)
    
    if args.mode in ["all", "throughput"]:
        benchmark_concurrent_throughput(args.url, num_concurrent=args.concurrent)
